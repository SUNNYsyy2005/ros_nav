...
class DWA(object):
    ...
    def plan(self, start_x, start_y, start_angel, goal_x, goal_y, goal_angle, vision, disRedius):
        ...
        for i in range(1, self.MAX_TRY_COUNT):
            ...
            vision.my_robot.x = goal_x
            vision.my_robot.y = goal_y
            vision.my_robot.orientation = goal_angle
    ...
    def stepPlan(self, cur, goal_x, goal_y, goal_angle, vision):
        ...
        for v in self.floatrange(MinV, MaxV, Interval_v):
            for w in self.floatrange(MinW, MaxW, Interval_w):
                ...
                #print(trajectory)
                # get obstacles
                obstacle_x, obstacle_y = self.getCurrentObstacles(vision.data)
                # get cost
    ...
    def EObstacleDist(self, trajectory, obstacle_x, obstacle_y):
        ...
        for i in range(0, len(trajectory)):
            ...
            for j in range(0, len(obstacle_x)):
                ...
                #print(minDis,obstacle_x,obstacle_y)
    ...
    def predictTrajectory(self, cur, v, w):
        ...
        return trajectory
    def getCurrentObstacles(self, AMCLLaserData):
        obstacle_x = []
        obstacle_y = []
        for i in range(0, AMCLLaserData.range_count):
            angle_min = 0
            angle_increment = math.pi*2/AMCLLaserData.range_count
            angle = angle_min+i*angle_increment
            x = AMCLLaserData.ranges[i]*math.cos(angle)
            y = AMCLLaserData.ranges[i]*math.sin(angle)
            obstacle_x.append(x)
            obstacle_y.append(y)
