...
def follow_the_gap_callback(data):
    ...
    left_start_angle = -60
    left_end_angle = -40
    right_start_angle = 40
    right_end_angle = 60
    close_threshold = 0.1 

    left_distances = [get_range(data, ang) for ang in range(left_start_angle, left_end_angle, STEP)]
    right_distances = [get_range(data, ang) for ang in range(right_start_angle, right_end_angle, STEP)]
    near = True
    bothnear = False
    if min(left_distances) < close_threshold and min(right_distances) > threshold:
        rospy.loginfo("Too close to an obstacle on the left side!")
        near = False
    if min(right_distances) < close_threshold and min(left_distances) > threshold:
        rospy.loginfo("Too close to an obstacle on the right side!")
        near = False
    if min(left_distances) < 2*close_threshold and min(right_distances) < 2*close_threshold and min(left_distances) > close_threshold and min(right_distances) > close_threshold:
        rospy.loginfo("Too close to obstacles on both sides!")
        bothnear = True
        return
    # 如果前方的障碍物距离大于阈值，机器人将继续直行
    if max(dis_list) > threshold and near:
        return
    
    rospy.loginfo("Find the gap")
    ...
    BACKWARD_TIME = 0.05
    if max(gap_list) < some_threshold and near:
        drive_msg = AckermannDriveStamped()
        drive_msg.drive.steering_angle = 0
        rospy.loginfo("Backward")
        drive_msg.drive.speed = -0.3  # negative speed for backward movement
        drive_pub.publish(drive_msg)
        rospy.sleep(BACKWARD_TIME)
        drive_msg.drive.speed = 0  # stop the robot
        drive_pub.publish(drive_msg)
        return
...
if __name__ == '__main__': 
  try:
    ...
    rospy.loginfo("Start following the gap")
    rospy.spin()
